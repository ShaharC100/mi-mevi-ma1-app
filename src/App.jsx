...
import { Share2, Plus, ArrowRight, Calendar, Users, Utensils, Info, Check, Trash2, Copy, Pencil, Clock, MapPin, MessageCircle, Mail, Facebook, X, Key, AlertTriangle } from 'lucide-react';
...
// תיקון קריטי: ניקוי לוכסנים ממזהה האפליקציה כדי למנוע שגיאות נתיב ב-Firebase
...
export default function App() {
  ...
  useEffect(() => {
    ...
    const unsubItems = onSnapshot(itemsRef, (snapshot) => {
      ...
      itemsData.sort((a, b) => (a.createdAt?.toMillis() || 0) - (b.createdAt?.toMillis() || 0));
      setItems(itemsData);
    ...
    <div className="space-y-8 animate-in fade-in duration-700">
      <div className="flex flex-col md:flex-row justify-between items-center bg-indigo-600 p-8 rounded-3xl text-white shadow-xl">
        <div className="mb-6 md:mb-0 text-center md:text-right">
          ...
          <p className="text-indigo-100">צרו דף למפגש, שתפו עם כולם וראו מי מביא מה.</p>
        </div>
      ...
      <div className="bg-white p-6 rounded-3xl border border-gray-200 shadow-sm">
        ...
        <div className="flex gap-3">
          <input type="text" value={joinId} onChange={(e) => setJoinId(e.target.value)} placeholder="הזינו קוד..." className="flex-1 px-5 py-3.5 rounded-2xl border border-gray-200 bg-gray-50 outline-none text-right font-mono" dir="rtl" />
          <button onClick={() => { if(joinId.trim()) onEventClick(joinId.trim()) }} className="bg-gray-800 text-white px-8 py-3.5 rounded-2xl font-black">הצטרפות</button>
        </div>
      </div>

      <div>
        <h3 className="text-lg font-black text-gray-800 mb-4 px-1">מפגשים פעילים</h3>
        {events.length === 0 ? (
          <div className="bg-white p-12 rounded-3xl border border-dashed border-gray-200 text-center text-gray-400">אין מפגשים עדיין.</div>
        ) : (
          <div className="grid gap-4 md:grid-cols-2">
            {events.map(event => (
              <div key={event.id} onClick={() => onEventClick(event.id)} className="bg-white p-6 rounded-3xl border border-gray-100 shadow-sm hover:shadow-lg cursor-pointer transition-all group">
                <h4 className="font-black text-xl text-gray-900 group-hover:text-indigo-600 mb-3">{String(event.eventName)}</h4>
                <div className="flex items-center text-gray-400 text-[11px] font-black gap-4">
  ...
  return (
    <div className="max-w-md mx-auto bg-white p-8 rounded-3xl shadow-2xl border border-gray-100">
      ...
      <form onSubmit={handleSubmit} className="space-y-5">
        <div>
          <label className="block text-[11px] font-black text-gray-400 mb-2 mr-1 uppercase tracking-widest">מה האירוע?</label>
          <input type="text" value={eventName} onChange={(e) => setEventName(e.target.value)} className="w-full px-4 py-3.5 rounded-2xl border border-gray-200 bg-gray-50 focus:bg-white outline-none font-bold" placeholder="למשל: יום הולדת לשחר" required />
        </div>
        <div className="grid grid-cols-2 gap-4">
          <div>
            <label className="block text-[11px] font-black text-gray-400 mb-2 mr-1 uppercase tracking-widest">תאריך</label>
            <input type="date" value={eventDate} onChange={(e) => setEventDate(e.target.value)} className="w-full px-4 py-3.5 rounded-2xl border border-gray-200 font-bold" required />
          </div>
          <div>
            <label className="block text-[11px] font-black text-gray-400 mb-2 mr-1 uppercase tracking-widest">מיקום</label>
            <input type="text" value={eventLocation} onChange={(e) => setEventLocation(e.target.value)} className="w-full px-4 py-3.5 rounded-2xl border border-gray-200 font-bold" placeholder="איפה זה?" />
          </div>
        </div>
        <div>
          <label className="block text-[11px] font-black text-gray-400 mb-2 mr-1 uppercase tracking-widest">שם המארגן</label>
          <input type="text" value={creatorName} onChange={(e) => setCreatorName(e.target.value)} className="w-full px-4 py-3.5 rounded-2xl border border-gray-200 font-bold" placeholder="משפחת לוי" required />
        </div>
        <div className="pt-6 flex flex-col gap-3">
          <button type="submit" disabled={isSubmitting} className="w-full bg-indigo-600 text-white py-4 rounded-2xl font-black shadow-lg active:scale-[0.98]">{isSubmitting ? 'מעבד...' : 'צור את המפגש'}</button>
          <button type="button" onClick={onCancel} className="w-full bg-gray-50 text-gray-500 py-3 rounded-2xl font-bold">ביטול</button>
        </div>
      </form>
    </div>
  );
...
function EventDetailsView({ event, items, user, isLoadingEvents }) {
  ...
  if (isLoadingEvents) return <div className="text-center py-20"><div className="animate-spin rounded-full h-12 w-12 border-b-2 border-indigo-600 mx-auto mb-4"></div><p className="font-bold text-gray-400">טוען נתונים...</p></div>;
  ...
  const handleDeleteItem = async (itemId) => {
    try { await deleteDoc(doc(db, 'artifacts', appId, 'public', 'data', 'potluck_items', itemId)); } catch (e) { console.error(e); }
  };
    ...
    <div className="space-y-8 animate-in fade-in duration-500">
      <div className="bg-white p-8 rounded-3xl shadow-sm border border-gray-100 flex flex-col md:flex-row justify-between items-center gap-6">
        <div className="space-y-4 text-center md:text-right">
          <h2 className="text-3xl font-black text-gray-900 tracking-tight">{String(event.eventName)}</h2>
          <div className="flex flex-wrap justify-center md:justify-start items-center gap-4 text-gray-500 text-sm font-bold">
            <span className="flex items-center gap-2 bg-indigo-50 text-indigo-600 px-4 py-2 rounded-xl"><Calendar className="w-4 h-4" />{String(event.eventDate)}</span>
        ...
        <button onClick={() => setIsShareModalOpen(true)} className="bg-indigo-600 text-white px-8 py-4 rounded-2xl font-black shadow-lg flex items-center gap-2 shrink-0">
          ...
          <div className="bg-white rounded-3xl shadow-2xl w-full max-w-sm p-8 relative">
            ...
            <div className="grid grid-cols-2 gap-6 mb-10">
              <a href={`https://api.whatsapp.com/send?text=${encodeURIComponent("הוזמנת למפגש: " + event.eventName + "\n" + getShareUrl())}`} target="_blank" rel="noopener noreferrer" className="flex flex-col items-center gap-3">
                <div className="w-16 h-16 bg-green-50 text-green-600 rounded-2xl flex items-center justify-center shadow-sm"><MessageCircle className="w-8 h-8" /></div>
      ...
      <div className="flex flex-col lg:flex-row gap-8 items-start">
        <div className="w-full lg:w-[320px] bg-white p-8 rounded-3xl border border-gray-100 shadow-sm sticky top-28">
        ...
        <div className="flex-1 w-full bg-white rounded-3xl border border-gray-100 shadow-sm overflow-hidden min-h-[500px]">
          <div className="p-6 border-b bg-gray-50/50 flex justify-between items-center">
             <h3 className="font-black text-gray-800 text-lg">רשימת הכיבוד המשותפת</h3>
             <span className="text-[10px] font-black bg-indigo-50 text-indigo-600 px-3 py-1.5 rounded-full border border-indigo-100">{items.length} פריטים</span>
          </div>
              ...
              <table className="w-full text-right border-collapse">
                <thead>
                  <tr className="bg-gray-50 text-gray-400 font-black text-[11px] uppercase tracking-widest border-b border-gray-100">
                    <th className="p-5">משפחה / שם</th>
                    <th className="p-5">סוג</th>
                    <th className="p-5">מנה / פריט</th>
                ...
                <tbody className="divide-y divide-gray-50">
                  {items.map((item) => (
                    <tr key={item.id} className="hover:bg-indigo-50/30 transition-colors group">
                      <td className="p-5">
                        <div className="font-black text-gray-900">{String(item.familyName)}</div>
                        {event.createdBy === item.userId && <span className="text-[9px] text-indigo-500 font-black uppercase tracking-tighter">מקים המפגש</span>}
                      </td>
                      <td className="p-5"><span className="bg-gray-100 text-gray-500 text-[10px] font-black px-2 py-1 rounded-lg">{String(item.mealComponent)}</span></td>
                      <td className="p-5 text-indigo-700 font-black">{String(item.dish)}</td>
                      <td className="p-5 text-center font-black text-gray-700 bg-gray-50/30">{String(item.quantity)}</td>
                      <td className="p-5 text-gray-400 text-sm font-bold italic max-w-[150px] truncate" title={item.notes}>{String(item.notes || '-')}</td>
                      <td className="p-5 text-left">
                        {user && (item.userId === user.uid || event.createdBy === user.uid) && (
                          <div className="flex gap-2 justify-end opacity-0 group-hover:opacity-100">
                            <button onClick={() => { setEditingItem(item); window.scrollTo({top: 0, behavior: 'smooth'}); }} className="bg-white text-gray-400 hover:text-indigo-600 p-2 rounded-xl shadow-sm border border-gray-100"><Pencil className="w-4 h-4" /></button>
                            <button onClick={() => handleDeleteItem(item.id)} className="bg-white text-gray-400 hover:text-red-500 p-2 rounded-xl shadow-sm border border-gray-100"><Trash2 className="w-4 h-4" /></button>
                          </div>
                        )}
                      </td>
                    </tr>
...
function AddItemForm({ eventId, user, items, editingItem, setEditingItem }) {
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-5">
      <div>
        <label className="block text-[10px] font-black text-gray-400 mb-2 mr-1 uppercase tracking-widest">שם משפחה</label>
        <input type="text" value={familyName} onChange={(e) => setFamilyName(e.target.value)} className="w-full px-4 py-3 rounded-2xl border border-gray-100 bg-gray-50 focus:bg-white outline-none font-bold" placeholder="משפחת כהן" required />
      </div>
      <div>
        <label className="block text-[10px] font-black text-gray-400 mb-2 mr-1 uppercase tracking-widest">כמות משתתפים</label>
        <input type="number" min="1" max="30" value={participantsCount} onChange={(e) => setParticipantsCount(e.target.value)} disabled={!!existing && !editingItem} className="w-full px-4 py-3 rounded-2xl border outline-none font-black disabled:bg-gray-100" required />
      </div>
      <div className="border-t border-gray-50 pt-6 mt-2">
        <label className="block text-[10px] font-black text-gray-400 mb-2 mr-1 uppercase tracking-widest">סוג המנה</label>
        <select value={mealComponent} onChange={(e) => setMealComponent(e.target.value)} className="w-full px-4 py-3 rounded-2xl border border-gray-100 bg-white font-bold mb-5 cursor-pointer">
          <option>עיקרית</option><option>תוספת</option><option>מנה צמחונית</option><option>עוגה</option><option>קינוח מתוק</option><option>פירות/ירקות</option><option>שתיה קלה</option><option>אלכוהול</option><option>אחר</option>
        </select>
        <label className="block text-[10px] font-black text-gray-400 mb-2 mr-1 uppercase tracking-widest">מה בתפריט?</label>
        <input type="text" maxLength={40} value={dish} onChange={(e) => setDish(e.target.value)} className="w-full px-4 py-3 rounded-2xl border border-gray-100 bg-white outline-none font-black text-indigo-700 shadow-sm" placeholder="סיר ממולאים ענק" required />
      </div>
      <div className="grid grid-cols-2 gap-4">
        <div>
          <label className="block text-[10px] font-black text-gray-400 mb-2 mr-1 uppercase tracking-widest">כמות</label>
          <input type="text" value={quantity} onChange={(e) => setQuantity(e.target.value)} className="w-full px-4 py-3 rounded-2xl border border-gray-100 bg-white outline-none font-black text-center" required />
        </div>
      </div>
      <div>
        <label className="block text-[10px] font-black text-gray-400 mb-2 mr-1 uppercase tracking-widest">הערות</label>
        <textarea maxLength={60} value={notes} onChange={(e) => setNotes(e.target.value)} className="w-full px-4 py-3 rounded-2xl border border-gray-100 bg-white outline-none font-bold resize-none h-20" placeholder="אלרגיות וכדומה..." />
      </div>
      <div className="pt-4 flex flex-col gap-3">
        <button type="submit" disabled={isSubmitting} className="w-full bg-indigo-600 text-white py-4 rounded-2xl font-black shadow-lg active:scale-[0.98] flex justify-center items-center gap-2 text-lg">
          {isSubmitting ? 'מעבד...' : (editingItem ? 'שמור שינויים' : 'הוסף לרשימה')}
          {!isSubmitting && !editingItem && <Utensils className="w-5 h-5" />}
        </button>
